...
m2a <- lm(lf_share_m_unw ~ spei_negative + spei_neg_spei_lag1 + spei_neg_spei_lag2+ spei_neg_spei_lag3+
            factor(STATE) + factor(year) + factor(STATE):year,
          data = data
          , weights =  pop_m_unw # pop_m_unw_2017
) 
...
# GENERAL POP
m1a <- lm(lf_share_unw ~ FI_state + FI_lag1 + FI_lag2 +
            factor(STATE) + factor(year) + factor(STATE):year,
          data = data_flood
          , weights = state_pop
) 

m1b <- lm(worker_share_unw ~ FI_state + FI_lag1 + FI_lag2 +
            factor(STATE) + factor(year) + factor(STATE):year,
          data = data_flood
          , weights = state_pop
) 

m1c <- lm(unemp_PS_unw ~ FI_state + FI_lag1 + FI_lag2 +
            factor(STATE) + factor(year) + factor(STATE):year,
          data = data_flood
          , weights = state_pop) 
...
stargazer(
  ...
  se = list(se1a[, 2], se1b[, 2], se1c[, 2]),
  p  = list(se1a[, 4], se1b[, 4], se1c[, 4]),
  title          = "Effect of Climate Shocks on Labor market",
  dep.var.labels = c("","",""),
  column.labels   = c("LF share", "Worker share", "Unemployment"),
  omit = c("factor\\(STATE\\)",
           "factor\\(year\\)",
...
stargazer(
  ...
  se = list(se2a[, 2], se2b[, 2], se2c[, 2],
            se3a[, 2], se3b[, 2], se3c[, 2]),
  p  = list(se2a[, 4], se2b[, 4], se2c[, 4],
            se3a[, 4], se3b[, 4], se3c[, 4]),
  title           = "Effect of Flash Flood Shocks on Labor Market Outcomes (Male and Female)",
  dep.var.caption = "",  # drops the italic "Dependent variable:" line
  dep.var.labels  = c("Male", "Male", "Male", "Female", "Female", "Female"),
  column.labels   = c("LF share", "Worker share", "Unemployment",
                      "LF share", "Worker share", "Unemployment"),
...
stargazer(
  m4a, m4b, m4c, m5a, m5b, m5c,
  se = list(se4a[, 2], se4b[, 2], se4c[, 2],
            se5a[, 2], se5b[, 2], se5c[, 2]),
  p  = list(se4a[, 4], se4b[, 4], se4c[, 4],
            se5a[, 4], se5b[, 4], se5c[, 4]),
  title           = "Effect of Climate Shocks on Labor Market Outcomes (Rural and Urban)",
  dep.var.caption = "",  # drops the italic "Dependent variable:" line
  dep.var.labels  = c("Rural", "Rural", "Rural", "Urban", "Urban", "Urban"),
  column.labels   = c("LF share", "Worker share", "Unemployment",
                      "LF share", "Worker share", "Unemployment"),
  column.separate = rep(1, 6),
  omit = c("factor\\(STATE\\)",
